...
def main(arg):
    for i in arg:
        ...
        while i < size * 12:
            ...
            if i == size*12 - 12:
                f2.write("key: "+str(key)+", offset: "+ str(offset) + ", content: " +content[offset-10272-size*12:].decode()+"\n")
            else:
                nextoffset = int.from_bytes(b[i+20:i+24],byteorder='little',signed=False)
            #print("offset:", offset)
            try:
                f2.write("key: "+str(key)+", offset: "+ str(offset) + ", content: " +content[offset-10272-size*12:nextoffset-10272-size*12].decode()+"\n")
            except UnicodeDecodeError:
                f2.write("Key: "+str(key)+", offset: "+ str(offset) + ", content: " +str(content[offset-10272-size*12:nextoffset-10272-size*12])+"\n")
